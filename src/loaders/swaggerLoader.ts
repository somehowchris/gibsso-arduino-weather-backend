...
import { getMetadataArgsStorage, MetadataArgsStorage } from '@mardari/routing-controllers';
...
export const swaggerLoader: MicroframeworkLoader = (settings: MicroframeworkSettings | undefined) => {
    if (settings && env.swagger.enabled) {
        ...
        const schemas = validationMetadatasToSchemas(validationMetadatas, {
            ...
            classTransformerMetadataStorage: defaultMetadataStorage as any,
        });

        const routingMetadataStorage: MetadataArgsStorage = getMetadataArgsStorage();
        const swaggerFile: any = routingControllersToSpec(routingMetadataStorage as any, {}, {
            servers: [
                {
